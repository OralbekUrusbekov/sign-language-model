...
class RealTimeSignLanguageRecognizer:
        ...
        if self.model_available:
            try:
                ...
                if hasattr(self.model, 'input_shape'):
                    ...
                    self.expected_sequence_length = self.model.input_shape[1]
                    print(f"✓ Expected sequence length: {self.expected_sequence_length}")
    ...
    def predict(self):
        ...
        try:
            ...
            current_pred = top3_labels[0]
            current_conf = top3_confidences[0]
            ...
            if len(self.prediction_history) >= 3:
                ...
                top_voted = counter.most_common(1)[0]

                # Use voting if confidence is low
                if current_conf < self.confidence_threshold:
                    predicted_label = top_voted[0] if top_voted[1] >= 2 else None
                else:
                    predicted_label = current_pred
            else:
                predicted_label = current_pred if current_conf >= self.confidence_threshold else None
    ...
    def process_frame(self, frame):
        ...
        # Show buffer status
        buffer_status = f"Frames: {len(self.feature_buffer)}/{self.sequence_length}"
        cv2.putText(frame, buffer_status, (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 0), 1)
